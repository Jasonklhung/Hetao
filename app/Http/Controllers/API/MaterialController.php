...
class MaterialController extends Controller
{
    public function get_material_detail(Request $request)
    {
    	...
    	elseif(empty($date) && !empty($dept)){
    		...
    		$org_id = $org->id;
    		$receive = Material::where('organization_id',$org_id);
    		$back = MaterialBack::where('organization_id',$org_id);
    	}
    	elseif(!empty($date) && empty($dept)){

    		$receive = Material::whereDate('date','=',$date);
    		$back = MaterialBack::whereDate('date','=',$date);
    	}
    	else{

    		$org = Organization::where('name',$dept)->get();
    		$org_id = $org->id;
    		$receive = Material::where('organization_id',$org_id)->whereDate('date','=',$date);
    		$back = MaterialBack::where('organization_id',$org_id)->whereDate('date','=',$date);
    	}
